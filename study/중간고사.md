...
UpdateData(true, false) : 다이얼로그 화면의 데이터를 메모리 변수에 저장, 메모리 변수의 값을 다이얼로그 화면에 출력
DoModal() : 모달형식으로 대화상자 띄우기
PostNcDestroy()
CClientDC
...
3. 주요 헤더파일 : #include "afx.h", "pch.h", "framework.h", "MainFrm.h", "PenDoc.h", "PenView.h" 등등
4. 시리얼라이즈하는 과정 : CObject에서 시작
...
5. 초기화 하는 코드 넣는 함수
6. 계산기 만들기
7. 펜만들기(sdi or 대화상자기반)
  1)펜의 크기 바꾸기
  2)색상표
  3)읽고 저장
  4)메뉴, 컨텍스트메뉴
  5)툴바 등
8. SetTimer(1, 1000, NULL); //1번에 1000은 1초간 타이머 호출을 설정 함.  
9. 확장 dll은 AFX_EXT_CLASS mfc 동적 연결 라이브러리-MFC 확장 DLL-헤더파일 새항목추가 math.h-
새프로젝트-mfc 동적 연결 라이브러리-공유dll-새항목추가
10. 정적라이브러리, 동적라이브러리 특징
11. 모덜리스, 모덜 다이어로그 차이, 특징 등
